...
public class NodeSetupTask extends Task {
  ...
  public NodeSetupTask(Settings settings) {
  ...
  public void execute() throws ExecutionException, IOException {
    ...
    if (mkdirCmd.getExitStatus() != 0) {
      throw new ExecutionException("Failed to create working dir on node: " + node.getHostAddr());
    }
    ...
    if (yumCmd.getExitStatus() == 0) {
      ...
      LOGGER.log(Level.INFO, "Node: " + node.getHostAddr() + " - OS Family: RedHat");
      ...
      // Ubuntu distribution
      LOGGER.log(Level.INFO, "Node: " + node.getHostAddr() + " - OS Family: Ubuntu");
    ...
    LOGGER.log(Level.INFO, "NodeSetup completed on node: " + node.getHostAddr());
  }
